...
1. Set a variable for the domain that will be used in the rest of this deployment.
   ...
   ```bash
   export DOMAIN_NAME_AKS_BASELINE="perun.karmf.net"
   ```
...
1. Generate a client-facing, self-signed TLS certificate.
   ...
   ```bash
   openssl req -x509 -nodes -days 365 -newkey rsa:2048 -out appgw.crt -keyout appgw.key -subj "/CN=${DOMAIN_NAME_AKS_BASELINE}/O=Vezha Perun"
   ```
